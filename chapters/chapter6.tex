...
Login and Get Started controls route to \url{/login} and \url{/register} respectively. Authenticated users visiting the root URL are redirected to the dashboard.
...
Upon authentication, the platform retrieves the user profile from \texttt{GET /api/v1/users/me}. Session state is maintained through Supabase AuthContext listeners. Users sign out via the header profile menu, invoking \texttt{POST /api/v1/auth/logout}. Deactivated accounts (\texttt{is\_active = false}) are redirected to the suspended-account page and cannot access protected routes.
...
Two roles are implemented: user and admin. Standard users access dashboard analytical modules. Administrators additionally access the SOC Nexus console at \url{/admin} for user management, threat feeds, analytics, system health, audit logs, and platform settings. Role enforcement is applied through RoleGuard components on protected routes.

Role changes are performed by administrators through the User Management interface in SOC Nexus. Standard users cannot access \url{/admin} routes.
...
The header displays platform branding, breadcrumb navigation, a notification bell, and profile controls. Notifications are fetched from \texttt{GET /api/v1/notifications/} with fifteen-second auto-refresh. Users may mark individual or all notifications as read.
...
The main dashboard at \url{/dashboard} serves as a workflow-oriented module launcher rather than a metrics dashboard.
...
Account status is verified via \texttt{GET /api/v1/users/dashboard/stats} on page load. Deactivated accounts are redirected before module content renders.
...
Infra Intelligence performs eight-stage infrastructure IOC enrichment for domain, IP, URL, hash, and email targets at \url{/dashboard/infra-investigations}. The pipeline aggregates DNS, WHOIS, reputation, certificate, and heuristic data to produce a composite infrastructure risk assessment suitable for threat hunting and incident triage.
...
The Threat Intelligence Lookup module (\url{/dashboard/threats}) provides rapid on-demand reputation checks using the same IOC types without full pipeline execution.
...
Security Scans provides VirusTotal-integrated URL and file reputation scanning at \url{/dashboard/scans}. The module supports asynchronous background scanning with automatic status polling, enabling analysts to submit multiple indicators concurrently.
...
Penetration Testing provides standalone website vulnerability scanning at \url{/dashboard/website-scanner}, accepting URL targets only. The scanner executes automated security tests and categorises findings by severity for prioritised remediation.
...
The review page at \url{/dashboard/website-scanner/review} loads cached JSON from localStorage for offline-capable result inspection.
...
Threat Modeling applies STRIDE methodology to application design profiles at \url{/dashboard/threat-modeling} without requiring live target scanning. The module supports proactive security assessment during application design and architecture review phases.
...
AI Malware Analysis performs static PE classification and AI-generated SOC reporting at \url{/dashboard/ai-malware-analysis} for .exe, .dll, and .sys files (max 512 MB). Files are analysed without execution; classification is based on extracted static PE features.
...
Reports History at \url{/dashboard/reports} aggregates completed outputs from Security Investigations, Penetration Testing, Security Scans, and Threat Modeling.
...
\midrule
Security Investigations & Investigations pipeline & \url{/dashboard/investigations/{id}} \\
Penetration Testing & Website scanner & \url{/dashboard/website-scanner} \\
Threat Scans & Security Scans & \url{/dashboard/scans} \\
Threat Models & Threat Modeling & \url{/dashboard/threat-modeling} \\
\bottomrule
\end{tabular}
\end{table}
...
The Profile page at \url{/dashboard/profile} displays read-only account information including full name, email, role, and account status retrieved from \texttt{GET /api/v1/users/me}.
