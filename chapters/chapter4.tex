...
This section presents the Use Case Diagram of the Threat-Intelligence Based Security Assessment (TIBSA) platform, which models the system’s functional behavior from a user interaction perspective. The use case diagram provides a high-level representation of how different actors interact with the platform’s core services, including scanning, reporting, dashboard management, and system administration. By illustrating these interactions, the diagram captures the functional requirements of the platform and clarifies the responsibilities of each actor within the system.

The design emphasizes a clear separation of privileges between guest users, registered users, and administrators, while also incorporating interactions with external services such as antivirus engines, authentication providers, email delivery services, and temporary storage systems. This structured representation supports system security, scalability, and maintainability by ensuring that each actor interacts only with the functionalities relevant to their role.

\clearpage
\begin{figure}[p]
\centering
\includegraphics[width=1.0\textwidth]{media/usecase1.jpg}
\caption{TIBSA Platform - Use Case Diagram (Part 1 of 3)}
\label{fig:usecase_part1}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=1.0\textwidth]{media/usecase2.jpg}
\caption{TIBSA Platform - Use Case Diagram (Part 2 of 3)}
\label{fig:usecase_part2}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=1.0\textwidth]{media/usecase3.jpg}
\caption{TIBSA Platform - Use Case Diagram (Part 3 of 3)}
\label{fig:usecase_part3}
\end{figure}
\clearpage


\subsection{Actors Overview}

The TIBSA platform defines a set of internal and external actors that represent all entities interacting with the system. The \texttt{Guest} actor represents unauthenticated users who access the platform’s public-facing features. Guests are permitted to view general information and perform limited scanning operations, allowing them to evaluate the platform without full system access.

The \texttt{Registered User} actor represents authenticated users who have full access to the platform’s primary functionalities. Registered users can submit scans, view detailed results, generate reports, manage personal dashboards, and access historical scan data. The \texttt{Admin} actor represents system administrators responsible for platform maintenance, monitoring, and configuration. Administrators manage antivirus engines, oversee system operations, update virus definitions, and administer user accounts.

In addition to internal actors, the system interacts with several external actors. The \texttt{Antivirus Engine} actor represents integrated scanning engines that analyze files, URLs, and hashes. The \texttt{Email Service} actor represents external services responsible for delivering scan reports to users. The \texttt{OAuth Provider} actor models third-party authentication services used for social login, while the \texttt{Temporary Storage} actor represents external storage used to hold uploaded files during the scanning process.

\subsection{Guest Use Cases}

Guest users interact with the TIBSA platform through a restricted set of use cases designed to provide basic functionality while preventing misuse of system resources. Guests can access the home page and documentation, register for a new account, and authenticate into the system. They are also allowed to upload a single file or submit a URL for scanning and view a basic summary of the scan results. Additionally, guests may access shared scan results through public links generated by registered users. These use cases provide limited exposure to platform capabilities while encouraging users to register for advanced features.

\subsection{Authentication Use Cases}

Authentication use cases govern access control, identity verification, and session management for both registered users and administrators. These use cases include standard login and logout functionality, password reset mechanisms, and social authentication through external OAuth providers. The Social Login use case extends the standard Login process by allowing users to authenticate using third-party identity providers as an optional alternative to traditional username and password credentials. This design enhances usability while maintaining secure authentication workflows.

\subsection{Scanning Use Cases}

Scanning operations constitute the core functionality of the TIBSA platform and are primarily available to registered users. These use cases include uploading files for analysis, submitting multiple files for batch scanning, submitting URLs for inspection, and performing hash-based reputation lookups. Registered users may optionally select specific antivirus engines to customize the scanning process. All scan submission use cases incorporate input validation to ensure correctness and security of user-provided data.

File-based scanning use cases additionally include temporary storage of uploaded files prior to analysis. The \texttt{Start Scan} use case represents the unified scanning pipeline that coordinates communication with external antivirus engines and initiates the analysis process. The \texttt{Select Antivirus Engines} use case extends the \texttt{Start Scan} use case, as engine selection is optional and available only to registered users.

\subsection{Scan Results, Reporting, and Sharing Use Cases}

After scan execution is completed, users can access a range of result analysis and reporting functionalities. These use cases include viewing detailed scan results, inspecting antivirus detection tables, analyzing file metadata, and reviewing in-depth analysis information. The platform supports aggregation of results from multiple antivirus engines and calculates detection ratios to provide a comprehensive assessment of potential threats.

Reporting and sharing functionalities enable users to generate scan reports, export reports in supported formats, create shareable public links, and send reports via email. Exporting and emailing operations both depend on report generation as a prerequisite. The process of sending reports via email involves interaction with an external email service, ensuring reliable delivery of analysis results.

\subsection{Dashboard and User Management Use Cases}

Registered users have access to personalized dashboard and account management functionalities that support effective monitoring and customization. These use cases include viewing the user dashboard, accessing scan history, reviewing statistical summaries, managing user profiles and preferences, and viewing system notifications. Together, these functionalities enhance usability, provide transparency into user activity, and allow users to track their interactions with the platform over time.

\subsection{Administrative Use Cases}

Administrative use cases support system governance, maintenance, and operational monitoring. Administrators can access the admin dashboard, view the list of available antivirus engines, monitor engine status, and manage engine configurations. Additional use cases include enabling or disabling antivirus engines, removing engines, updating virus definitions, and viewing update progress and logs. Administrators are also responsible for monitoring overall system operations and managing user accounts. These use cases ensure system reliability, security, and traceability of maintenance activities.

\subsection{Use Case Relationships}
This describes the relationships between use cases in the TIBSA platform, illustrating how complex system behavior is decomposed into reusable and well-structured interactions. Use case relationships are employed to model mandatory and optional functionality, reduce redundancy, and improve the clarity of the overall behavioral design. By explicitly defining these relationships, the use case diagram ensures consistent execution of shared processes while supporting flexibility through optional extensions based on user roles and system conditions.

\subsubsection{Include Relationships}
Include relationships represent mandatory sub-processes that are always executed as part of a parent use case. In the TIBSA platform, input validation is included in all scan submission use cases to ensure the correctness, integrity, and security of user-provided data. Temporary file storage is included in file-based scanning operations to securely hold uploaded files during analysis. Report generation is included in both export and email operations, as reports must be generated before they can be distributed or shared. Additionally, result aggregation and detection ratio calculation are included in detailed result views and report generation to provide comprehensive and consolidated threat assessment outcomes.

\subsubsection{Extend Relationships}
Extend relationships represent optional or conditional behavior that enhances base use cases without altering their core functionality. In the TIBSA platform, the Social Login use case extends the standard Login use case by allowing users to authenticate through external OAuth providers as an alternative to traditional credentials. Similarly, antivirus engine selection extends the Start Scan use case, enabling registered users to customize scanning operations by choosing specific engines when such customization is required. These extend relationships improve system flexibility while maintaining a clear and reusable use case structure.
...
% Class Diagram - Split across two facing pages
\clearpage
\begin{figure}[p]
\centering
\includegraphics[width=1.1\textwidth,height=0.88\textheight,keepaspectratio,clip,trim=0cm 0cm 8cm 0cm]{media/class_diagram.jpg}
\caption{TIBSA Platform - Comprehensive Class Diagram (Part 1 of 2)}
\label{fig:ch4_class_diagram_part1}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=1.1\textwidth,height=0.88\textheight,keepaspectratio,clip,trim=8cm 0cm 0cm 0cm]{media/class_diagram.jpg}
\caption{TIBSA Platform - Comprehensive Class Diagram (Part 2 of 2)}
...
The URLScanning class serves as the central component responsible for analyzing web addresses for potential security threats such as phishing attempts, malware distribution, and malicious content. This class encapsulates all functionality related to URL-based threat detection and analysis. The class maintains several key attributes including \texttt{scanID} which serves as a unique identifier assigned to each URL scanning operation for tracking and reference purposes, \texttt{targetURL} representing the web address submitted for security analysis, \texttt{scanStatus} indicating the current state of the scanning operation (pending, in-progress, completed, or failed), and \texttt{createdAt} which records the timestamp when the scan request was initiated.

The class provides essential methods for URL analysis operations. The \texttt{initiateScan()} method initiates the URL scanning process by sending the target URL to the analysis engines. The \texttt{getScanResults()} method retrieves the complete analysis results including threat indicators and reputation scores. The \texttt{cancelScan()} method terminates an ongoing scan operation and releases associated resources.

\subsection{AuthenticationHandler Class}

The AuthenticationHandler class manages all aspects of user authentication and session management within the TIBSA platform. It serves as the primary interface between the user interface layer and the authentication service, ensuring secure access control throughout the system. The class attributes include \texttt{sessionID} which provides a unique token identifying the current user session, \texttt{userEmail} containing the email address associated with the authenticated user, \texttt{loginAttempts} as a counter tracking failed login attempts for security monitoring, \texttt{isAuthenticated} as a flag indicating whether the current session is authenticated, \texttt{mfaEnabled} indicating whether multi-factor authentication is enabled for the account, and \texttt{lastLogin} recording the timestamp of the user's most recent successful login.

The class implements comprehensive authentication methods. The \texttt{registerUser(email, password)} method creates a new user account with the provided credentials. The \texttt{authenticateUser(email, password)} method validates user credentials and issues authentication tokens. The \texttt{validateToken(token)} method verifies the validity and expiration status of authentication tokens. The \texttt{refreshToken(refreshToken)} method issues new access tokens using valid refresh tokens. The \texttt{verifyMFA(code)} method validates multi-factor authentication codes. The \texttt{logout()} method terminates the current session and invalidates associated tokens.

\subsection{UserServices Class}

The UserServices class provides comprehensive user account management functionality, handling user profiles, preferences, and account-related operations. This class acts as the service layer for all user-centric operations beyond authentication. The class maintains attributes including \texttt{userID} as the unique identifier for the user account, \texttt{userName} representing the display name of the user, \texttt{userEmail} containing the primary email address associated with the account, \texttt{userRole} indicating the role assigned to the user (Admin, Analyst, User, Guest), \texttt{profileData} as a container for additional user profile information, and \texttt{createdAt} recording the account creation timestamp.

The class provides several methods for user management. The \texttt{getProfile(userID)} method retrieves the complete profile data for a specified user. The \texttt{updateProfile(userID, data)} method updates user profile information with the provided data. The \texttt{deleteAccount(userID)} method permanently removes a user account and associated data. The \texttt{getUserRole(userID)} method returns the current role assignment for a user. The \texttt{updatePreferences(userID, prefs)} method modifies user preferences and settings.

\subsection{ScanServices Class}

The ScanServices class orchestrates all security scanning operations within the platform, coordinating between file analysis, URL scanning, and threat intelligence services. This class serves as the central hub for initiating, managing, and tracking scan operations across the entire system. The class attributes include \texttt{scanID} as the unique identifier for the scan operation, \texttt{scanType} indicating the classification of the scan (file, URL, or hash lookup), \texttt{targetIdentifier} representing the subject of the scan (file path, URL, or hash value), \texttt{priority} indicating the processing priority level for the scan request, \texttt{status} showing the current operational status of the scan, and \texttt{initiatedBy} containing the user ID of the requesting user.

The class implements comprehensive scanning methods. The \texttt{createScan(type, target, userID)} method initializes a new scan operation with specified parameters. The \texttt{getScanStatus(scanID)} method returns the current status and progress of a scan. The \texttt{getScanResults(scanID)} method retrieves completed scan results and analysis data. The \texttt{cancelScan(scanID)} method terminates an in-progress scan operation. The \texttt{queueScan(scan)} method adds a scan to the processing queue for asynchronous execution. The \texttt{aggregateResults(scanID)} method combines results from multiple analysis engines into a unified report.

\subsection{FileAnalysis Class}

The FileAnalysis class handles the processing and security analysis of uploaded files, implementing both static and dynamic analysis techniques. This class integrates with sandbox environments, machine learning engines, and threat intelligence services to provide comprehensive file security assessments. The class maintains attributes including \texttt{fileID} as the unique identifier assigned to the uploaded file, \texttt{fileName} containing the original name of the uploaded file, \texttt{fileHash} representing the SHA-256 hash of the file content for integrity verification, \texttt{fileSize} indicating the size of the file in bytes, \texttt{mimeType} containing the MIME type classification of the file, \texttt{uploadDate} recording the timestamp of file upload, and \texttt{analysisStatus} indicating the current state of the analysis process.

The class provides essential methods for file analysis. The \texttt{uploadFile(file)} method processes and stores an uploaded file for analysis. The \texttt{calculateHash(file)} method computes cryptographic hashes (MD5, SHA-1, SHA-256) for the file. The \texttt{extractMetadata(fileID)} method extracts file properties and metadata information. The \texttt{initiateStaticAnalysis(fileID)} method begins static analysis of file structure and content. The \texttt{initiateDynamicAnalysis(fileID)} method submits the file to sandbox environment for behavioral analysis. The \texttt{getAnalysisReport(fileID)} method retrieves the comprehensive analysis report for a file.

\subsection{PentestJobService Class}

The PentestJobService class manages penetration testing operations within the TIBSA platform, handling job scheduling, execution coordination, and results collection for automated security assessments. The class attributes include \texttt{jobID} as the unique identifier for the penetration testing job, \texttt{targetScope} defining the target scope for testing, \texttt{testModules} as a collection of testing modules to be executed, \texttt{scheduledTime} indicating the scheduled execution time for the job, and \texttt{jobStatus} representing the current status of the pentest job.

The class implements methods for penetration testing management. The \texttt{createJob(scope, modules)} method creates a new penetration testing job with specified parameters. The \texttt{scheduleJob(jobID, time)} method schedules a job for future execution. The \texttt{executeJob(jobID)} method initiates the execution of a pentest job. The \texttt{getJobResults(jobID)} method retrieves the findings and results from a completed job. The \texttt{cancelJob(jobID)} method cancels a scheduled or running job.

\subsection{ThreatIntelService Class}

The ThreatIntelService class provides integration with threat intelligence sources, enabling the platform to enrich security analysis with external threat data, indicators of compromise (IOCs), and reputation information. The class maintains attributes including \texttt{queryID} as the unique identifier for threat intelligence queries, \texttt{indicatorType} indicating the type of indicator being queried (IP, domain, hash, URL), \texttt{indicatorValue} containing the actual indicator value for lookup, \texttt{sources} as a list of threat intelligence sources to query, and \texttt{lastUpdated} recording the timestamp of the most recent data refresh.

The class provides methods for threat intelligence operations. The \texttt{lookupIOC(indicator)} method queries threat intelligence sources for indicator information. The \texttt{getReputation(target)} method retrieves reputation scores for domains, IPs, or URLs. The \texttt{enrichData(scanResults)} method augments scan results with threat intelligence context. The \texttt{syncFeeds()} method synchronizes local threat data with external feed sources. The \texttt{queryMISP(indicator)} method queries the MISP platform for related threat events.

\subsection{MLEngineService Class}

The MLEngineService class encapsulates the machine learning capabilities of the TIBSA platform, providing intelligent threat detection through trained models for phishing classification, malware detection, and anomaly identification. The class attributes include \texttt{modelID} as the identifier for the machine learning model in use, \texttt{modelVersion} indicating the version number of the deployed model, \texttt{modelType} representing the classification of the model (phishing, malware, anomaly), \texttt{accuracy} containing the current accuracy metric of the model, and \texttt{lastTrained} recording the timestamp of the most recent model training.

The class implements machine learning methods. The \texttt{classifyURL(url, features)} method classifies a URL as phishing or legitimate. The \texttt{classifyFile(fileFeatures)} method determines malware probability for a file. The \texttt{extractFeatures(input)} method extracts relevant features for model input. The \texttt{runInference(features)} method executes model inference on provided feature vectors. The \texttt{getConfidenceScore(prediction)} method returns the confidence level of a prediction.

\subsection{AccountManager Class}

The AccountManager class handles subscription management, usage tracking, and account tier administration, integrating with billing services to manage user entitlements and resource allocation. The class maintains attributes including \texttt{accountID} as the unique identifier for the billing account, \texttt{subscriptionPlan} indicating the current subscription tier (Freemium, Pro, Enterprise), \texttt{usageQuota} representing the maximum allowed resource usage for the account, \texttt{currentUsage} tracking the current resource consumption count, \texttt{billingCycle} indicating the billing period (monthly, annual), and \texttt{expirationDate} recording the subscription expiration date.

The class provides methods for account management. The \texttt{getSubscriptionDetails(accountID)} method retrieves current subscription information. The \texttt{updateSubscription(accountID, plan)} method modifies the subscription plan. The \texttt{trackUsage(accountID, resource)} method records resource usage for billing purposes. The \texttt{checkQuota(accountID)} method verifies if the account has remaining quota. The \texttt{generateInvoice(accountID)} method creates billing invoices for the account.

\subsection{ThreatAnalysisEngine Class}

The ThreatAnalysisEngine class represents the core threat modeling functionality of the TIBSA platform, automating the generation of threat models, STRIDE analysis, and risk assessments based on system architecture inputs. The class attributes include \texttt{modelID} as the unique identifier for the threat model, \texttt{architectureInput} containing the system architecture description in JSON or YAML format, \texttt{dfdGenerated} as a flag indicating whether DFD has been generated, \texttt{riskScore} representing the calculated overall risk score for the system, and \texttt{analysisDate} recording the timestamp of the threat analysis execution.

The class implements comprehensive threat modeling methods. The \texttt{generateDFD(architecture)} method creates Data Flow Diagrams from architecture specifications. The \texttt{performSTRIDE(dfd)} method executes STRIDE threat analysis on the model. The \texttt{mapMITRE(threats)} method maps identified threats to MITRE ATT\&CK framework. The \texttt{calculateRisk(threats, controls)} method computes risk scores based on likelihood and impact. The \texttt{generateReport(modelID)} method produces comprehensive threat modeling reports.

\subsection{ITScanAPIWrapper Class}

The ITScanAPIWrapper class provides a unified interface for interacting with external scanning APIs and antivirus engines, abstracting the complexity of multiple vendor integrations behind a consistent API. The class maintains attributes including \texttt{apiEndpoint} containing the base URL for the external API, \texttt{apiKey} as the authentication key for API access, \texttt{engineName} representing the name of the integrated scanning engine, \texttt{isActive} indicating whether the integration is currently active, and \texttt{rateLimitRemaining} tracking the remaining API calls within the rate limit period.

The class provides methods for external API integration. The \texttt{submitScan(target)} method submits a target to the external scanning service. The \texttt{getResults(scanID)} method retrieves scan results from the external service. The \texttt{checkStatus(scanID)} method queries the status of an external scan operation. The \texttt{validateAPIKey()} method verifies the validity of the configured API key.

\subsection{FTPThreagileInput Class}

The FTPThreagileInput class manages the input processing for the Threagile threat modeling engine, handling file transfers and format conversions required for automated threat model generation. The class attributes include \texttt{inputID} as the unique identifier for the input request, \texttt{inputFormat} indicating the format of the input file (JSON, YAML), \texttt{validationStatus} representing the status of input validation, and \texttt{processingQueue} indicating the queue assignment for processing.

The class implements input processing methods. The \texttt{validateInput(data)} method validates input data against schema requirements. The \texttt{transformFormat(input, targetFormat)} method converts input between supported formats. The \texttt{queueForProcessing(inputID)} method adds validated input to the processing queue.

\subsection{SecurityControlsEvaluator Class}

The SecurityControlsEvaluator class assesses the effectiveness of existing security controls against identified threats, providing gap analysis and recommendations for security improvements. The class maintains attributes including \texttt{evaluationID} as the unique identifier for the evaluation session, \texttt{controlsInventory} as a list of security controls under evaluation, \texttt{threatsCovered} representing the threats addressed by current controls, and \texttt{gapsIdentified} containing the security gaps requiring attention.

The class provides methods for security controls evaluation. The \texttt{evaluateControls(controls, threats)} method assesses control effectiveness against threats. The \texttt{identifyGaps(evaluation)} method identifies areas lacking adequate protection. The \texttt{recommendControls(gaps)} method suggests controls to address identified gaps. The \texttt{calculateCoverage(controls)} method computes the percentage of threats mitigated.

\subsection{ReportGenerator Class}

The ReportGenerator class handles the creation and formatting of all reports within the TIBSA platform, including scan reports, threat model documentation, and executive summaries. The class maintains attributes including \texttt{reportID} as the unique identifier for the generated report, \texttt{reportType} indicating the classification of the report (scan, threat model, executive), \texttt{outputFormat} representing the desired output format (PDF, Word, JSON), \texttt{generatedAt} recording the timestamp of report generation, and \texttt{storageURL} containing the location where the report is stored.

The class implements report generation methods. The \texttt{compileReportData(sourceID)} method aggregates data required for report generation. The \texttt{generatePDF(data)} method creates PDF format reports with visualizations. The \texttt{generateWord(data)} method produces Word format documentation. The \texttt{exportJSON(data)} method exports report data in JSON format for integration.
...
\subsection{Scan\_Request Table}
...
\subsection{Antivirus\_Engine Table}
...
\subsection{Engine\_Result Table}
...
\subsection{Threat\_Model Table}
...
\subsection{System\_Component Table}
...
\subsubsection{Multiplicity Summary}
